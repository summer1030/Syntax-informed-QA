...
def squad_convert_examples_to_features(
    ...
    threads=1,
    tqdm_enabled=True,
    graph_data_path="./data/squad_files/constituency_graphs/",
):
    """
    ...
    if return_dataset == "pt":
        ...
        if not is_training:
            ...
            dataset = [SquadData(input_ids=torch.tensor(f.input_ids, dtype=torch.long),
                            ...
                            # t2t_edge_index=torch.transpose(torch.tensor(f.t2t_edge_index,dtype=torch.long),0,1) if f.t2t_edge_index!=[] else torch.tensor([[0],[0]],dtype=torch.long),
